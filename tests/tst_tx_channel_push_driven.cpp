...
private slots:
    ...
    // ── No async dispatches occur after setRunning(true) ────────────────────
    //
    // Proves the QTimer is gone: after starting the channel and waiting for
    // 30 ms (more than enough for the old 5 ms timer to fire 6× times), no
    // sendTxIq calls should have occurred without an explicit tickForTest.

    void noTimer_doesNotFireWithoutPush()
    {
        TxChannel ch(kChannelId, kBufSize, kBufSize);
        MockConnection conn;
        ch.setConnection(&conn);
        ...
        // Spin event loop — old timer would have fired ~6× by now.
        QTest::qWait(30);

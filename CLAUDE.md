...
### Current Phase: v0.3.2 shipped 2026-05-05. Four pieces of work landed between v0.3.1 (2026-05-03) and v0.3.2: **3M-3a-iii DEXP/VOX + AMSQ** Complete (PR #185 + bench polish through PR #194 review): full WDSP DEXP DSP port from Thetis `create_dexp` + `xdexp` driver (v0.3.1 had no DEXP DSP; the wrappers existed but the underlying DSP didn't), VOX wired to MOX state machine via WDSP `pushvox` callback, dedicated Setup → Transmit → DEXP/VOX page mirroring Thetis `tpDSPVOXDE` 1:1 (grpDEXPVOX + grpDEXPLookAhead + AMSQ NereusSDR-spin), DexpPeakMeter widget for live VOX/DEXP peak strip, MicProfileManager +11 DEXP keys, TxApplet VOX row (relocated from PhoneCwApplet), PhoneCwApplet VOX/DEXP rows wired with right-click DspParamPopup, retired VoxSettingsPopup + legacy VoxDexpSetupPage; **HL2 mi0bot RF/Tune Power parity (#175, PR #194)** RF Power slider 0-90 step 6 (16-step output attenuator, 0.5 dB/step), Tune Power slider 0-99 step 3 (33 sub-steps; 0-51 = DSP audio gain modulation, 52-99 = PA attenuator territory), per-band Tune Power grid in dB on HL2, polymorphic per-band tune-power clamp, Setup → Transmit → Power gains `grpPATune` Tune group with 3 drive-source radios + TX TUN Meter combo + Fixed-mode spinbox + Reset Tune Power Defaults button, mi0bot HL2 audio-volume formula `(hl2Power * gbb / 100) / 93.75` (`mi0bot-Thetis console.cs:47660-47673 + 47775-47778 [v2.10.3.13-beta2]`); **PA Setup parity + safety hotfix (#167)** Setup → PA pages rebuilt to full Thetis parity (PA Gain page with 14-band gain grid + 14×9 drive-step adjust matrix + auto-cal sweep state machine; Watt Meter page with chkPAValues toggle + Reset PA Values button; PA Values page closes 4-label gap from v0.3.1 via public PaTelemetryScaling helpers + peak/min tracking), kernel doubles as PA over-drive safety hotfix (per-band PA gain compensation now applied at audio output level via Thetis `SetPowerUsingTargetDBM` port at `console.cs:46645-46762 [v2.10.3.13]`, preventing over-drive on high-gain finals at low TUNE-slider positions; resolves K2GX field report on ANAN-8000DLE >300W at slider 50% / 80m), TwoToneController routes through new math kernel, ATT-on-TX safety scaffolding pre-staged for 3M-4 PureSignal; and a **persistence + stability fix tail** per-band TX RF Power (#192), OC matrix pin persistence (#191, PR #195), mic-jack PTT default behavior (#182), TUN-off cold-start (#177), audio bus reconfigure on Settings open (#172), NR3 dev-build model path (#180/#184, thanks KM4BLG), HL2 connect-init parity (#189) covering #153 cold-start + SSB MOX, A-ATT label/N2ADR default (#174), P1 ADC overload byte offset (#176, PR #187), HL2 PA fwd/rev power 16.7x over-read corrected, step-attenuator MOX gating, SWR foldback topology correction (now applies to IQ scalar only, not wire byte). v0.3.1 (2026-05-03) prior context: **3M-1 SSB TX** (PR #152), **Phase 3Q connection workflow refactor + chrome layer** (PR #158), **3M-3a-i** TX EQ + Leveler + ALC, **3M-3a-ii** CFC + CPDR + CESSB + Phase Rotator (MicProfileManager 91 keys, 19 of 21 factory profiles ported verbatim), **ParametricEq widget port** (PR #159), **Plan 4 ui-polish-foundation epic** (PR #166). HL2 ATT/filter safety audit closed in v0.3.1. Pre-emphasis remains de-scoped from 3M-3a-ii to 3M-3b (FM-mode follow-up). **Next on this epic: 3M-2 CW TX**: sidetone, firmware keyer, QSK/break-in. Absorbs HL2 CWX bit-3 follow-up (`networkproto1.c:1247-1252 [@c26a8a4]`; desk-review B3, "HL2 firmware uses bit 3 of I-low byte for CWX PTT, non-HL2 boards don't"). After 3M-2: 3M-3b (FM pre-emphasis), 3M-4 PureSignal, then 3F multi-panadapter and the longer-tail 3H/3J/3K planned phases.
...
| **3M-3a-iv: Anti-VOX Cancellation Feed** | Closes the gap left in 3M-3a-iii where the anti-VOX gain control was plumbed end-to-end but `SendAntiVOXData` was never called, leaving `antivox_data` zero and the cancellation silent. 4 new WDSP wrappers (`SetAntiVOXSize` / `Rate` / `DetectorTau` / `SendData`) on `TxChannel`. `RxDspWorker` emits `antiVoxSampleReady` per chunk + `bufferSizesChanged` from `setBufferSizes`. `TxWorkerThread` queued slots `onAntiVoxSamplesReady` / `setAntiVoxBlockGeometry` / `setAntiVoxDetectorTau` / `setAntiVoxRun` (new wrapper that flips `m_antiVoxRun` atomic gate). `MoxController::setAntiVoxTau(int ms)` slot with NaN-sentinel idempotency, ms->s scaling. `TransmitModel::antiVoxTauMs` Q_PROPERTY (range 1-500 ms, default 20, persisted per-MAC under `AntiVox_Tau_Ms`). RadioModel wires 4 new queued connections + rewires the existing `antiVoxSourceWhatRequested` connect through the new `TxWorkerThread::setAntiVoxRun` wrapper so the atomic gate actually flips. Setup → Transmit → DEXP/VOX gains a "Tau (ms)" QSpinBox in a new grpAntiVOX group (tooltip "Time-constant used in smoothing Anti-VOX data" verbatim from Thetis `setup.designer.cs:44681 [v2.10.3.13]`). WdspEngine constant fix: anti-VOX tau default `0.01` → `0.02` to match Thetis `udAntiVoxTau.Value=20`. Single-RX direct pump (no aamix); aamix port deferred to 3F multi-pan. Bench-verification matrix at `docs/architecture/phase3m-3a-iv-verification/README.md` (5 rows × ANAN-G2 + HL2). 12 GPG-signed commits, all pre-existing tests green plus 21 new test cases extending `tst_tx_channel_vox_anti_vox` / `tst_mox_controller_anti_vox` / `tst_transmit_model_anti_vox` / `tst_dexp_vox_setup_page` / `tst_rx_dsp_worker_thread` / `tst_rx_dsp_worker_buffer_sizing` / `tst_tx_worker_thread`. PR #TBD. | **Complete (pending bench)** |

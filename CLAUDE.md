...
### Current Phase: v0.2.3 shipped 2026-04-24 — RX epic + Alex antennas complete. **3M-1 SSB TX shipped via PR #152 (2026-04-29).** **Phase 3Q connection workflow refactor + chrome layer in PR review on `feature/phase3q-connection-workflow-refactor` 2026-04-30 (PR #158)** — single ConnectionState state machine, unicast probe (RadioDiscovery::probeAddress works across Layer-3 VPNs), Add Radio dialog rebuilt with 16-SKU model picker, ConnectionPanel polish (state pills + Last Seen + ↻ Scan + status strip + auto-open/auto-close timing), Radio menu rework, spectrum disconnect overlay, saved-radio age-out exemption, auto-connect-on-launch failure path, macKey migration; **plus chrome layer**: status-bar redesign (RxDashboard with BadgePair drop ladder + StationBlock + AdcOverloadBadge + OverflowChip), SVG icon system on StatusBadge (10 icons replacing Unicode glyph prefixes), TitleBar ConnectionSegment with min-filtered RTT, CPU System/App toggle, ship-default calibration shifted -12 dB, PA voltage formula corrected per Thetis convertToVolts (live ANAN-G2 reads 13.3 V), PSU widget dropped after Thetis source-first audit. **3M-3a-i (TX EQ + Leveler + ALC) Complete (pending bench) — 13 GPG-signed commits, 246/246 tests green.** **3M-3a-ii (CFC + CPDR + CESSB + Phase Rotator) Complete (pending bench) on `feature/phase3m-3-tx-processing` 2026-04-30 — 17 GPG-signed commits stacked on 3M-3a-i; 253/253 tests green; awaiting JJ bench on ANAN-G2.** Pre-emphasis was de-scoped from 3M-3a-ii to 3M-3b (FM-mode work) per master design §7.2 ("run as written"). The 3M-3a-ii TxCfcDialog landed scalar-complete (profile combo + global spinboxes + 30 per-band spinboxes for F/COMP/POST-EQ across 10 bands) but is visually spartan; full Thetis-faithful `ucParametricEq` widget port (replaces both TxCfcDialog and TxEqDialog visualizations) is queued as a follow-up sub-PR — see `docs/architecture/phase3m-3a-ii-cfc-eq-parametriceq-handoff.md`. Next on this epic after ParametricEq lands is **3M-3a-iii (DEXP/VOX + AMSQ)**; 3M-2 CW TX still deferred until after the full 3M-3 epic ships AND the HL2 ATT/filter safety audit closes (HL2 TX bench was deferred during 3M-1c after JJ flagged ATT/filter code paths that could damage the radio if engaged — see PR #152 rows 58-60).

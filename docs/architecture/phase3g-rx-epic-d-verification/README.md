...
| 6 | View → Band Plan → IARU Region 1 | Strip relabels live; segment count + boundaries change to R1 plan; the radio button next to "IARU Region 1" is checked | |
| 7 | View → Band Plan → IARU Region 2 / IARU Region 3 / RAC (Canada) | All four non-default regions load and render; radio button selection follows | |
| 8 | View → Band Plan → Off | Strip disappears entirely; spectrum extends to bottom of widget; "Off" radio button checked | |
| 9 | View → Band Plan → Large (or Huge) | Strip thickens; labels render larger; radio button moves with selection | |
...
- [`src/gui/MainWindow.cpp:4098-4130`](https://github.com/ten9876/AetherSDR/blob/0cd4559/src/gui/MainWindow.cpp) (`View → Band Plan` menu)
- [`resources/bandplans/*.json`](https://github.com/ten9876/AetherSDR/tree/0cd4559/resources/bandplans) (5 region files)

## IA note

Bandplan controls live exclusively under **View → Band Plan** (5 sizes
+ flat region list, separator-divided). There is intentionally no
Setup-side UI for this — matches AetherSDR's IA, and a single surface
avoids the "two places to change the same thing" problem.
